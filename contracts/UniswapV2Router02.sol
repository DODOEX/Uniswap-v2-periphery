...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, uint256[] memory fees, address _to) internal virtual {
        ...
        uint256[] memory _fees = fees;

        for (uint i; i < _path.length - 1; i++) {
            (address input, address output, uint256 fee) = (_path[i], _path[i + 1], _fees[i]);
            (address token0,) = UniswapV2Library.sortTokens(input, output);
            ...
            address token1 = _path[i + 2];
            fee = _fees[i + 1];
            address to = i < _path.length - 2 ? UniswapV2Library.pairFor(factory, output, token1, fee) : _to;
